...
const MySqlOperations_1 = require("./database/mysql/MySqlOperations");
...
async function main() {
    ...
    const dbOperations = new MySqlOperations_1.MySqlOperations();
    try {
    ...
    const server = new index_js_1.Server({ name: "mysql-mcp-server", version: "1.0.0" }, { capabilities: { tools: {} } });
    // 3. 도구 목록 분리: 외부 파일에서 도구 정의를 가져와 사용
    server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => {
        console.error('Tools list requested');
        return { tools: tools_1.toolDefinitions };
    });
